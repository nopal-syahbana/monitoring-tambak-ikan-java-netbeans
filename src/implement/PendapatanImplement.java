...
public class PendapatanImplement {
    ...
    public List<PendapatanEntity> getListData() {
        ...
        try {
            String sqlSelect = "SELECT pendapatan.*, tambak.nama, customer.nama_pt FROM pendapatan JOIN tambak ON pendapatan.tambak_id = tambak.id "
            ...
            while (resultSet.next()) {
                ...
                pendapatanEntity.setIdTambak(resultSet.getInt("pendapatan.tambak_id"));
                pendapatanEntity.setNamaCustomer(resultSet.getString("customer.nama_pt"));
                pendapatanEntity.setNamaTambak(resultSet.getString("tambak.nama"));
    ...
    public List<PendapatanEntity> getListDataByParameter(String searchParameter) {
        ...
        try {
            String sqlSelect = "SELECT pendapatan.*, tambak.nama, customer.nama_pt FROM pendapatan JOIN tambak ON pendapatan.tambak_id = tambak.id "
                    + "JOIN customer ON customer.id = pendapatan.customer_id "
                    + "WHERE tambak.nama LIKE '%" + searchParameter + "%' OR customer.nama_pt LIKE '%" + searchParameter + "%' ORDER BY id";
    ...
    public String insertData(PendapatanEntity pendapatanEntity) {
        ...
        try {
            ...
            preparedStatement.setInt(2, pendapatanEntity.getIdTambak());
    ...
    public String updateData(PendapatanEntity pendapatanEntity) {
        ...
        PreparedStatement preparedStatement = null;
        try {
            String sqlUpdate = "UPDATE pendapatan SET tambak_id = ?, customer_id = ?, jumlah_panen = ? "
                    + ", harga_kg = ?,total_pendapatan = ?,ket = ?, updated_at = ? WHERE id = ?";

            preparedStatement = Koneksi.getConnection().prepareStatement(sqlUpdate);
            preparedStatement.setInt(1, pendapatanEntity.getIdTambak());
