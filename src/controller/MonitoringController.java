...
import implement.MonitoringImplement;
import table.MonitoringDetailTableModel;
import table.MonitoringTableModel;
...
public class MonitoringController {
        ...
        try {
            if (MonitoringView.textIdKebun.getText().equals("")) {
                ...
                if (MonitoringView.comboProgress.getSelectedIndex() == 1) {
                    monitoringDetailEntity.setProgressTambak("perawatan");
                } else if (MonitoringView.comboProgress.getSelectedIndex() == 2) {
                    monitoringDetailEntity.setProgressTambak("siap_panen");
                } else if  (MonitoringView.comboProgress.getSelectedIndex() == 3) {
                    monitoringDetailEntity.setProgressTambak("panen_gagal");
                } else if  (MonitoringView.comboProgress.getSelectedIndex() == 4) {
                    monitoringDetailEntity.setProgressTambak("penanaman_kembali");
                } else if  (MonitoringView.comboProgress.getSelectedIndex() == 5) {
                    monitoringDetailEntity.setProgressTambak("proses_panen");
                }  else if  (MonitoringView.comboProgress.getSelectedIndex() == 6) {
                    monitoringDetailEntity.setProgressTambak("selesai_panen");
                } else  {
                    monitoringDetailEntity.setProgressTambak("proses_tanam");
                }

                monitoringDetailEntity.setId(Integer.valueOf(MonitoringView.labelId.getText()));
                monitoringDetailEntity.setTambakId(Integer.valueOf(MonitoringView.textIdKebun.getText()));
    ...
    private void loadData(MonitoringView monitoringView) {
        try {
            List<TambakEntity> list = monitoringImplement.getListData();
    ...
    public void searchData(MonitoringView monitoringView) {
        try {
            ...
            List<TambakEntity> list = monitoringImplement.getListDataByParameter(seacrhParameter);
            monitoringTableModel.setList(list);
        } catch (Exception error) {
    ...
    public void buttonCariKebunAction(MonitoringView monitoringView) {
        try {
            ...
            TambakCariView.labelView.setText("MONITORING");
    ...
    public void tableAction(MonitoringView monitoringView) {
        MonitoringView.tabelKebun.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent e) {
                ...
                if(row != -1) {
                    //ISI METHOD DISINI
                    TambakEntity kebunEntity = monitoringTableModel.get(row);
